...
- **配套上位机(Windows)**:源码见 [`上位机/`](上位机/),编译好的 `Fnos-connect.exe` 见 [Releases](../../releases)。主要**配合本项目原创的 USB-C 虚拟网卡功能**——板子完全无网时,用 USB-C 连电脑、设网卡 IP(默认 `10.42.0.2`,板子固定 `10.42.0.1`)即可直连管理页;也能一键扫描局域网/异地组网里的飞牛设备并打开。设 IP 需管理员权限。
